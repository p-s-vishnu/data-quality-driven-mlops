...
def split(X, y, val_size, test_size=None, max_size=None, random_state=1, shuffle=False):
    ...
    N = X.shape[0]
    if shuffle: idx = np.random.permutation(N)
    ...
    idx_train = idx[:N-test_size-val_size]
    idx_val = idx[N-test_size-val_size: N-test_size]
    idx_test = idx[N-test_size: ]
...
def build_real_dataset(data_dir, dataset, val_size=1000, test_size=None,
    ...
    X_train_mv, _, _, _, _, _ = \
        split(X_full_dirty, y_full_dirty, val_size, test_size,
              max_size=max_size, random_state=random_state, shuffle=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_size', default=1407, type=int)
