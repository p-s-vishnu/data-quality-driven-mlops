...
def export_log_file_as_csv(data_dir=Path('data/')):
    def parse_log_file(log_file_path):
        with open(log_file_path, "r") as log_file:
            for line in log_file:
                if ("[DEBUG]" in line) and ("pollution parameters" in line):
                    start, end = line.index('{'), line.index('} ')
                    if start != end:
                        pollution_parameters = line[start:end+1]
                        filepath = line[end+len(" from "):].strip()
                        yield pollution_parameters, filepath

    log_file_dir = data_dir / 'logs/experiment_*.log'
    out = []
    for log_file in glob(str(log_file_dir)):
        # parse_log_file(log_file_path)
        for ret in parse_log_file(log_file):
            pollution_param, filepath = ret
            out.append([pollution_param, filepath])
    DataFrame(out, columns=['pollution_parameter', 'filepath']).to_csv('polluted_file_mapping.csv', index=False)
