...
def modelpicker(predictions, labelspace, budget, oracle=None):
    """
    :param predictions:
    :param labelset:
    :param budget:
    ...
    for t in np.arange(1, num_instances+1, 1):
        ...
        if zt == 1:
            if oracle is not None:
                print("Label for the instance with ID "+str(shuffled_indices[t-1])+":",str(oracle[shuffled_indices[t-1]]))
                label_t = oracle[shuffled_indices[t-1]]
            else:
